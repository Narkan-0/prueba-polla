...
def obtener_fixture_completo():
    return [
        ...
        # --- FECHA 2 (Partidos 25 al 48) — ORDENADO CRONOLÓGICAMENTE ---
        ...
        # --- FECHA 3 (Partidos 49 al 72) — ORDENADO CRONOLÓGICAMENTE ---
        ...
        # --- DIEZISEISAVOS DE FINAL (Partidos 73 al 88 — COBERTURA TOTAL DE LLAVES) ---
        ...
        # --- OCTAVOS DE FINAL (Partidos 89 al 96 — COMPLETOS) ---
        ...
        # --- CUARTOS DE FINAL (Partidos 97 al 100 — COMPLETOS) ---
        ...
        # --- SEMIFINALES (Partidos 101 y 102 — COMPLETOS) ---
...
with tabs[1]:
    ...
    puntero_3 = df_tabla.iloc[2]["Participante"].upper() if len(df_tabla) > 3 else "POR DEFINIR"
    ...
    with st.form(key=f"form_seguro_{usuario}_{filtro_fia}"):
        ...
        for part in partidos_visibles:
            ...
            with col_inputs:
                ...
                with c_in1:
                    g_l = st.number_input("GL", min_value=0, max_value=15, value=pred_l_val, placeholder="-", key=f"l_{usuario}_{pid}", disabled=bloquear_casilla, label_visibility="collapsed")
                with c_in2:
                    g_v = st.number_input("GV", min_value=0, max_value=15, value=pred_v_val, placeholder="-", key=f"v_{usuario}_{pid}", disabled=bloquear_casilla, label_visibility="collapsed")
            with col_v:
        ...
        if boton_guardar:
            ...
            st.session_state["mensaje_exito"] = f"¡Excelente {usuario}, tus pronósticos activos de la {filtro_fia} fueron guardados correctamente!"
...
with tabs[3]:
    ...
    def estilo_filas_finalizadas(row):
        if row["Estado"] == "🔒 FINALIZADO":
            return ['background-color: rgba(71, 85, 105, 0.35); color: #cbd5e1; font-style: italic;'] * len(row)
        elif row["Estado"] == "⏱️ EN CURSO / EN JUEGO":
            return ['background-color: rgba(186, 18, 60, 0.25); color: #fda4af; font-weight: bold;'] * len(row)
        return [''] * len(row)
    ...
    if pass_input == PASSWORD_ADMIN:
        ...
        if accion_admin == "📝 Cargar Resultados Oficiales":
            ...
            if st.button("🔄 ACTUALIZAR MARCADORES MUNDIALES", use_container_width=True):
                ...
                st.session_state["mensaje_exito"] = "¡Superpoderes activados! Los marcadores oficiales se guardaron y las posiciones fueron recalculadas con éxito."
            ...
            for part in partidos_admin:
                pid = str(part["id"])
                pred_actual = datos["pronosticos"].get(jugador_a_editar, {}).get(pid, {})
                
                st.markdown(f"**Partido #{pid} ({part['grupo']}): {part['local']} vs {part['visita']}**")
                c_il, c_iv = st.columns(2)
                
                with c_il:
                    val_admin_l = pred_actual.get("l") if "l" in pred_actual else None
                    g_l = st.number_input(f"Goles Local", min_value=0, max_value=15, value=val_admin_l, placeholder="-", key=f"admin_l_{jugador_a_editar}_{pid}")
                with c_iv:
                    val_admin_v = pred_actual.get("v") if "v" in pred_actual else None
                    g_v = st.number_input(f"Goles Visita", min_value=0, max_value=15, value=val_admin_v, placeholder="-", key=f"admin_v_{jugador_a_editar}_{pid}")
            ...
            if st.button(f"💾 SALVAR CARTILLA DE {jugador_a_editar.upper()}", use_container_width=True):
                ...
                for pid, scores in respuestas_temporales_admin.items():
                    if scores["l"] is not None and scores["v"] is not None:
                        ...
                        datos["pronosticos"][jugador_a_editar].pop(pid, None)
                    
                guardar_datos(datos)
                
                st.session_state["mensaje_exito"] = f"¡Modificación forzosa de {jugador_a_editar} inyectada y guardada con éxito!"
